...
def run_temporal_holdout(
    ...
    if len(held_idx) < 5:
        raise RuntimeError(f"Too few held-out BGCs ({len(held_idx)}) after cutoff {cutoff}")
    ...
    # Random-holdout control: repeatedly carve a same-size random slice out of
    # the reference corpus, refit on the remainder, score the slice the same
    # way. This is the null distribution against which the true temporal
    # signal must be judged (removes "being held out" as a confound).
    rng = np.random.default_rng(seed)
    control_means = []
    for _ in range(n_controls):
        perm = rng.permutation(len(ref_idx))
        ctrl_pos = perm[: len(held_idx)]
        rest_pos = perm[len(held_idx) :]
    ...
    # Also draw one representative random-control sample (not just means) for the plot
    perm = rng.permutation(len(ref_idx))
    ctrl_pos, rest_pos = perm[: len(held_idx)], perm[len(held_idx) :]
    Z_rest, rest_knn_mean, _, _ = _fit_reference(X_ref[rest_pos], k=k)
    scaler_c = StandardScaler(with_mean=False).fit(X_ref[rest_pos])
    pca_c = PCA(n_components=Z_rest.shape[1], random_state=42).fit(scaler_c.transform(X_ref[rest_pos]))
    ...
    plot_df = pd.concat(
        [
            pd.DataFrame({"novelty": control_novelty_sample, "group": "random control\n(held from reference)"}),
